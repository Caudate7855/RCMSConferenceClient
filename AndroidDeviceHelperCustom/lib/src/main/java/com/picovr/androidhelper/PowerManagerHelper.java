...
public class PowerManagerHelper extends AndroidHelper {
    ...
    public void wakeupAtTime(int hours, int minutes, int seconds){
        ...
        // Перенос на следующий день если указанное время меньше текущего
        if(calendar.getTimeInMillis() < System.currentTimeMillis())
            calendar.setTimeInMillis(calendar.getTimeInMillis() + 86400000); // + 24 часа в миллисекундах

        wakeupAtTimeMilliseconds(calendar.getTimeInMillis());
    }
    ...
    public void wakeupAtTimeMilliseconds(long milliseconds){
        ...
        PendingIntent alarmIntent = PendingIntent.getBroadcast(mContext, alarmId, intent, PendingIntent.FLAG_ONE_SHOT);

        alarmManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, milliseconds, alarmIntent);
    }
