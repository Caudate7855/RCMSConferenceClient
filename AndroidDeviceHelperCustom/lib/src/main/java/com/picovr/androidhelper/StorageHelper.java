...
public class StorageHelper extends AndroidHelper {
    ...
    public void setKeyConfig(String contents){

        Log.e("","AndroidDeviceHelper: java setKeyConfig " + contents);

        try {

            File file = new File("/data/local/tmp/SystemKeyConfig.prop");

            if(!file.exists()){
                file.createNewFile();
            }

            FileWriter fileWriter = new FileWriter("/data/local/tmp/SystemKeyConfig.prop", false);
            ...
            Log.e("","AndroidDeviceHelper: java setKeyConfig " + e.toString());
            e.printStackTrace();
        }
    }
}
