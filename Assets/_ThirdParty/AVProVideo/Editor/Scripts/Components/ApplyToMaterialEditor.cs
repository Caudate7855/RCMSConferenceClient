...
namespace RenderHeads.Media.AVProVideo.Editor
{
	...
	public class ApplyToMaterialEditor : UnityEditor.Editor
	{
		...
		private SerializedProperty _propMaterials;
		...
		void OnEnable()
		{
			...
			_propMaterials = this.CheckFindProperty("_materials");
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.PropertyField(_propMaterials);

			bool hasKeywords = false;
			int texturePropertyIndex = 0;
			if (_propMaterials != null && _propMaterials.arraySize > 0)
			{
				for (int i = 0; i < _propMaterials.arraySize; i++)
				{
					SerializedProperty propMaterial = _propMaterials.GetArrayElementAtIndex(i);
					
					if(propMaterial.objectReferenceValue == null) continue;
					
					Material mat = (Material)propMaterial.objectReferenceValue;
