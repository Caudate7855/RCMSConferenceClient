...
public class Pvr_UnitySDKEditor : MonoBehaviour
{
    ...
    private void Start()
    {
        // по умолчанию в редакторе при запуске игры включен режим "делить экран на два глаза"
        // здесь по умолчанию включаем полноэкранный режим для редактора
        EnableVEmodel();
    }
    ...
    private void UpdateSimulatedSensor()
    {
        ...
        /*else if (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)) // TODO убран наклон головы по оси Z
        {
            rolled = true;
