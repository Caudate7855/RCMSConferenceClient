...
public class Pvr_ControllerModuleInit : MonoBehaviour
{
    ...
    void Awake()
    {
        _controllersHandler = GetComponentInParent<ControllersHandler>();
        ...
        if (Pvr_ControllerManager.Instance.LengthAdaptiveRay)
        {
            ...
            rayLine.GetComponent<LineRenderer>().endWidth = 1f;
#else
    ...
    private IEnumerator ShowOrHideRay(int id)
    {
        ...
        if (moduleState)
        {
            var state = Controller.UPvr_GetMainHandNess() == id &&
                        Controller.UPvr_GetControllerState(id) == ControllerState.Connected;
            dot.SetActive(state);
